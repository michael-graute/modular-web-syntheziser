...
- [x] T032 [US5] Add a `KARPLUS_STRONG` case to the private `createControls()` method in `src/canvas/CanvasComponent.ts` instantiating `Knob` controls for Frequency, Damping, Tone and a `Dropdown` for Mode, plus display wiring to instantiate `KarplusStrongDisplay` and a display-name map entry (per project memory: `componentLayout.ts` alone does not create controls) — depends on T014 (Foundational sizing), T031
- [x] T033 [US5] Confirm MIDI Learn works on Frequency, Damping, and Tone controls (FR-011) by verifying the new `Knob` instances from T032 use the same MIDI-mappable control class/pattern as existing components — depends on T032 — *(confirmed by code inspection: `CanvasComponent.handleControlMouseDown()` intercepts any `Knob`/`Slider` in `this.controls` generically when MIDI Learn is active, and `MidiEngine` reads scaling via the base `SynthComponent.getParameterRange()`, which `KarplusStrong` inherits unmodified — no per-component MIDI wiring was needed)*
...
- [~] T034 [P] Run `npm run lint` and fix any warnings across all new/modified files (`src/worklets/`, `src/components/generators/KarplusStrong.ts`, `src/canvas/CanvasComponent.ts`, `src/canvas/displays/KarplusStrongDisplay.ts`, `src/utils/componentLayout.ts`, `src/components/registerComponents.ts`, `src/core/types.ts`) — *(N/A: this project has no `lint` script defined in `package.json`, despite CLAUDE.md referencing `npm test && npm run lint`; `tsc --noEmit` is used as the equivalent static-check gate instead and passes clean on every file touched by this feature)*
- [x] T035 [P] Run `vitest run` and confirm all new tests pass and existing suites are unaffected (no regressions in `Oscillator`/`ADSREnvelope`/`SlewLimiter` tests from shared-pattern changes) — 77 files / 1689 tests passing
- [x] T038 [P] Write an automated stress test in `tests/components/generators/KarplusStrong.test.ts` (or a dedicated `KarplusStrong.stress.test.ts`) that calls `pluck()` at a simulated rate of ≥10 times/sec against a mock `AudioContext`/worklet stub and asserts: no thrown errors, no `NaN`/`Infinity` produced in the output signal, and no unbounded growth in internal delay-line values — automates SC-007 rather than relying solely on manual verification — *(added `tests/worklets/karplus-strong-dsp.test.ts` "rapid re-trigger numerical stability" suite: simulates the actual worklet pluck/process loop — matching the corrected one-period-back feedback indexing found during manual testing — at 10/sec and at worst-case every-block re-trigger rates, asserting all samples stay finite and bounded. Complements T015/T018's existing mock-level "rapid re-trigger sends multiple pluck messages" test in `KarplusStrong.test.ts`, which covers dispatch but not DSP numerical stability.)*
- [x] T036 Manually execute `specs/034-karplus-strong-oscillator/quickstart.md` end-to-end in the dev server (`npm run dev`), confirming SC-001 through SC-008 hold, including rapid re-trigger stability at ≥10 triggers/sec (SC-007, cross-checked against T038's automated result) and no perceptible latency/responsiveness degradation (SC-008) — *(verified live in a real browser: trigger→pluck→decay (SC-001), pitch tracking via Keyboard CV (SC-002), Damping/Tone/Mode knobs and dropdown all wired and audible, waveform display renders live data. Found and fixed two real bugs along the way — see T008 note. SC-007/SC-008 covered by T038's automated test plus qualitative confirmation that playback stayed responsive during testing.)*
- [x] T037 Verify backward compatibility: load a pre-existing patch file (containing no Karplus-Strong component) and confirm it still loads without error (per plan.md Constraints: "Patch format changes must be backward-compatible") — *(added `tests/persistence/karplus-strong-backward-compat.test.ts`: a legacy-style patch fixture with no Karplus-Strong component serializes/deserializes without error and preserves all existing component types/parameters exactly, confirming `PatchSerializer`'s type-agnostic design isn't affected by the new `ComponentType.KARPLUS_STRONG` registration.)*
