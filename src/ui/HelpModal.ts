/**
 * HelpModal - Modal dialog for displaying help and user manual
 */

import { Modal } from './Modal';

/**
 * Modal for displaying help documentation
 */
export class HelpModal extends Modal {
  ...
  /**
   * Setup modal content with navigation and sections
   */
  private setupContent(): void {
    ...
    // Add close button
    this.clearButtons();
    this.addButton('Close', () => this.close(), 'primary');
  }
  ...
  private updateContent(): void {
    ...
    const nav = this.getBody().querySelector('div > div');
    if (nav) {
      const buttons = nav.querySelectorAll('button');
      buttons.forEach((button, index) => {
        const sections = ['quickstart', 'interface', 'components', 'connections', 'keyboard', 'patches', 'shortcuts', 'tips'];
